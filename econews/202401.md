...
- Trezor has [disclosed](https://blog.trezor.io/trezor-security-update-stay-vigilant-against-potential-phishing-attack-bb05015a21f8) on Jan 20 an incident where the contact information of 66,000 users was exposed .

- The unauthorized access occurred on a third-party support platform, hence usrs who interacted with Trezor's support team since December 2021 had their data exposed.
...
- While assets were not directly exposed, phishing attempts would presumably attempt to gain access to the Trezor user's funds by for example obtaining their seed phrase. There were a [reported](https://fixedfloat.com/blog/news/weekly-2024-01-27#post_h_7) 41 incidents immediately following the announcement where users were approached in attempts to obtain seed words.
...
- This is the first release to incorporate a mobile BR GUI implementation (bruig) which can be installed as an apk file on android systems.

- Improvements to bruig include a choice of 3 font sizes, a Reset All KX button to prompt BR to exchange keys with all known users ("which should un-wedge clients in strange situations"), a Subscribe button to folow a user's comment feed, and an "Unrestrict Battery" option for mobile which has been observed to improve message retrieval performance.
...
- The release is accompanied by important server policy changes to "allow for various updates to existing restrictions (eg. max message size of 1mb)".
